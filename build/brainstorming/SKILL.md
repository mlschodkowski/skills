...
Use `$super-normal` for questions, options, and the recommendation when
the reader needs clearer wording.
...
1. **Read the local context.** Inspect the relevant files, docs, or
   recent changes before asking broad questions.
...
5. **Write the design.** Scale it to the work: a short paragraph for
   ...
   data and control flow, edge cases, and verification. Write a durable
   design doc only when that is useful or requested.

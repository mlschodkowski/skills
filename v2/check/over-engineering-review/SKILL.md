...
description: >
  Review a diff only for accidental complexity. List what to delete or
...
List complexity in the change that has no corresponding requirement. Do
not edit files unless asked. For behavior, security, or performance, use
`$code-review`.
...
Hunt accidental complexity. Abstractions must earn existence; boundaries
should follow real ownership or change lines; remove what has no job.
Prefer the same behavior in fewer clear lines — not minimalism that
strips necessary completeness.
...
with one caller; structure added for a future that is not here.

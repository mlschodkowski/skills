...
Prefer an explicit mechanism you can run over speculation about
structure.

When exploring, use the project's domain glossary for the modules you
touch, and check ADRs in that area. Prefer forms and seams already
...
everything and grep." Preserve only the information that separates
causes; hide the rest.
...
Change carefully: fix the shared owner of the mistake when you can.
Before adding a guard at one call site, check the other callers.
...
being locked down — the boundary does not match reality. Note it. Fix
anyway if you can, and say what would make the next one lockable.
...
after the fix is in, not before. Completeness of the lock-down matters
more than a tidy-looking patch.

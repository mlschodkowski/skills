...
Check that the happy path, failure path, and owner are easy to find;
names and control flow are direct; policy, state, side effects, and
external boundaries have clear owners; wrappers that only forward once
or hide a decision are called out; errors, logs, and metrics help
operators without leaking secrets; tests would fail if the bug returned;
the change is no larger than the problem.

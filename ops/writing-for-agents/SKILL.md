...
output. The document should read like a specialist's procedure, not a
methodology.
...
6. Preserve natural reasoning and voice. Use `$super-normal` when reader
   clarity is the problem. Add a constraint only when it changes
   behavior or prevents a demonstrated failure.
